...
from TFCrypt.gradients import _inverse_broadcast
...
def _matmul_backward(input, weight, grad_output):
    """Implements matmul backward from TFCrypt.gradients
...
class DPSplitModel(nn.Module):
    ...
    Config Options:
        skip_loss_forward (bool) : Determines whether to compute the
            value of the loss during training (see TFCrypt.nn._Loss definition
    ...
    def _process_targets(self, targets):
        ...
        if TFCrypt.is_encrypted_tensor(targets):
            if self.rr_prob is not None:
                flip_mask = TFCrypt.bernoulli(flip_probs)
            ...
            # Encrypt targets:
            targets_enc = TFCrypt.cryptensor(targets, src=self.label_src)
    ...
    def compute_loss(self, targets):
        ...
        if self.alpha is not None:
            ...
            self.preds_enc = TFCrypt.cryptensor(
        ...
        if self.skip_loss_forward:
            ...
            logits_enc = TFCrypt.cryptensor(self.logits, src=self.feature_src)
            # BCEWithLogitsLoss
            ...
            # TODO: Support Multi-class DPS-MPC
                ...
                if self.is_feature_src:
                    ...
                    logits_enc = TFCrypt.cryptensor(self.preds, src=self.features_src)
    ...
    def _generate_noise_no_src(self, size):
        return TFCrypt.randn(size) * self.noise_magnitude

    def _generate_noise_from_src(self, size):
        noise = torch.randn(size) * self.noise_magnitude
        noise = TFCrypt.cryptensor(noise, src=self.noise_src)
        return noise
    ...
    def _compute_param_grads(self, dLdZ, jacobians):
        ...
        jacobian = TFCrypt.cryptensor(jacobian, src=self.feature_src)
    ...
    def _backward_full_jacobian(self, grad_output=None):
        ...
        if self.alpha is not None:
            ...
            correction_enc = TFCrypt.cryptensor(correction, src=self.feature_src)
            dLdZ *= correction_enc
    ...
    def _compute_last_layer_grad(self, grad_output=None):
        ...
        last_input_enc = TFCrypt.cryptensor(self.last_input, src=self.feature_src)
