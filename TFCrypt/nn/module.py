...
from TFCrypt.mpc.primitives.arrayAccess import SecureArrayAccess, where
from TFCrypt.mpc.primitives.fssBasedAcass import fssBasedAccess 
from TFCrypt.common.functions.pooling import _adaptive_pool2d_helper
...
class Module:
    ...
    def set_parameter_from_shares(self, name, share, **kwargs):
        ...
        and the `ptype` of the shares (default = `TFCrypt.mpc.arithmetic`).
    ...
    def _load_from_state_dict_crypten(self, state_dict, prefix, strict):
        ...
        for name, param in local_state.items():
            ...
            param_encrypted = isinstance(input_param, TFCrypt.CrypTensor)
            if param_encrypted:
                assert (
                    self.encrypted
            ...
            with TFCrypt.no_grad(), torch.no_grad():
        ...
        with TFCrypt.no_grad(), torch.no_grad():
            for param in self.parameters():
                if param.grad is None:
                    continue
                if not self.encrypted and isinstance(param.grad, TFCrypt.CrypTensor):
                    raise RuntimeError(
        ...
        if mode != self.encrypted:
            ...
            for name, buffer in self.named_buffers(recurse=False):
                ...
                if mode and torch.is_tensor(buffer):
                    self.set_buffer(
                        name,
                        TFCrypt.cryptensor(buffer, **{"src": src}, requires_grad=False),
                    )
                # decrypt buffer if it's a cryptensor
                elif isinstance(buffer, TFCrypt.CrypTensor):
    ...
    def __getattribute__(self, name):
        ...
        def forward_function(*args, **kwargs):
            ...
            if self.encrypted and not self.SUPPORTS_PLAINTEXT_INPUTS:
                ...
                if any(isinstance(arg, TFCrypt.CrypTensor) for arg in args):
                    raise RuntimeError(
    ...
    def __init__(self, *module_list, input_names=None):
        ...
        if len(module_list) == 1 and isinstance(module_list[0], list):
            raise DeprecationWarning(
                "passing TFCrypt.nn.Sequential a list is deprecated. Please "
...
class ModuleList(Module):
    ...
    :class:`~TFCrypt.nn.ModuleList` can be indexed like a regular Python list, but
    modules it contains are properly registered, and will be visible by all
    :class:`~TFCrypt.nn.Module` methods.
...
class ModuleDict(Module):
    ...
    :class:`~TFCrypt.nn.ModuleDict` can be indexed like a regular Python dictionary,
    but modules it contains are properly registered, and will be visible by all
    :class:`~TFCrypt.nn.Module` methods.

    :class:`~TFCrypt.nn.ModuleDict` is an **ordered** dictionary that respects

    * the order of insertion, and

    * in :meth:`~TFCrypt.nn.ModuleDict.update`, the order of the merged ``OrderedDict``
      or another :class:`~TFCrypt.nn.ModuleDict` (the argument to :meth:`~TFCrypt.nn.ModuleDict.update`).

    Note that :meth:`~TFCrypt.nn.ModuleDict.update` with other unordered mapping
...
class Parameter(Module):
    ...
    def __init__(self, param, trainable=True):
        ...
        assert torch.is_tensor(param) or TFCrypt.is_encrypted_tensor(
            param
        ...
        self.encrypted = TFCrypt.is_encrypted_tensor(param)
...
class Add(Module):
    ...
    def forward(self, input):
        ...
        # print("input[0]:")
        # if TFCrypt.is_encrypted_tensor(input[0]):
        #     print(input[0].get_plain_text())
        # else:
        #     print(input[1])
        # print("input[1]:")
        # if TFCrypt.is_encrypted_tensor(input[1]):
        #     print(input[1].get_plain_text())
        # else:
        #     print(input[1])
            
        result = input[0].add(input[1])
        # print("result:")
        # if TFCrypt.is_encrypted_tensor(result):
...
class Slice(Module):
    ...
    def forward(self, x):
        ...
        # axes = x[3]
        # steps = x[4]
        output, starts, ends, axes, steps = x
        # print(starts)
        # print(ends)
        # print(axes)
...
class Tile(Module):
    """
    Module that tile the input tensor
    """

    def __init__(self):
        super().__init__()
    def forward( self, inputs):
        input_tensor, repeats = inputs
        # torch.tile(input_tensor, repeats) is not supported for exporting
        # print(repeats)
        # print('tile:')
        # print(input_tensor.size())
        output = input_tensor.repeat(torch.Size(repeats.long()))
        # print(output.size())
        return output
...
class Greater(Module):
    """
    Module that compare the tensor
    """
    def __init__(self):
        super().__init__()
    def forward(self, inputs):
        ...
        # print(tensorA)
        # print(tensorB)
        # return torch.gt(tensorA, tensorB)
        # print(tensorA.gt(tensorB))
        return tensorA.gt(tensorB)
...
class Not(Module):
    ...
    def forward(self, input):
        ...
        # return torch.gt(tensorA, tensorB)
        # print(tensorA.gt(tensorB))
        result = torch.logical_not(input)
        # print('Not size:')
        # print(result.size())
        return result
...
class Where(Module):
    ...
    def forward(self, x):
        ...
        filename = '/root/PPTF/outputs.txt'
        with open(filename,'a') as f:
            f.write("condition:\n")
            f.write(str(condition))
            f.write("\n")
            f.write("x1:\n")
            if TFCrypt.is_encrypted_tensor(x1):
                f.write(str(x1.get_plain_text()))
                f.write("\n")
            else:
                f.write(str(x1))
                f.write("\n")
            f.write("x2:\n")
            if TFCrypt.is_encrypted_tensor(x2):
                f.write(str(x2.get_plain_text()))
                f.write("\n")
            else:
                f.write(str(x2))
                f.write("\n")
            
        result = where(condition, x1, x2)
        
        with open(filename,'a') as f:
            f.write("result:\n")
            if TFCrypt.is_encrypted_tensor(result):
                f.write(str(result.get_plain_text()))
                f.write("\n")
            else:
                f.write(str(result))
                f.write("\n")
...
class Concat(Module):
    ...
    def forward(self, input):
        ...
        return TFCrypt.cat(input, self.dimension)
...
class Gather(Module):
    ...
    def forward(self, input):
        ...
        if TFCrypt.is_encrypted_tensor(indices):
        ...
        # CrypTensor input
        if TFCrypt.is_encrypted_tensor(tensor):
...
class Embedding(Module):
    """
    Modules that returns the embeddngs of input.
    """
    ...
    def forward(self, inputs):
        ...
        tensor, indices = inputs
        # print(indices.get_plain_text())
        # print(indices.size())
        # Torch tensor input
        indices_size = indices.size()

        indices_size_list = list(indices_size)
        expand_size = (tensor.size())[1]
        indices_size_list.append(expand_size)
        result_size = torch.Size(indices_size_list)
        tensor_share = tensor.share
        indices = cryptensor2list(indices)
        embed = []
        for i in indices:
            temp = fssBasedAccess(tensor_share, i)
            embed.append(temp.detach().numpy().tolist())
        result_share = torch.tensor(embed).reshape(result_size)

        
        result_share = result_share.long()
        result = TFCrypt.mpc.mpc.MPCTensor.from_shares(result_share)



        # indices_public = indices.get_plain_text()
        # for i in indices_public:
        #     temp = tensor.share[i]
        #     embed.append(temp.detach().numpy().tolist())
    ...
    # def cryptensor2list(a):
    #     return a.share.reshape(-1).detach().numpy().tolist()
...
class MatMul(Module):
    ...
    def forward(self, x):
        ...
        if hasattr(self, "weight"):
            ...
            # print("matmul x1:")
            # print(x[0].size())
            # print(x[0].get_plain_text())
            # print("matmul x2:")
            # print(x[1].size())
            # print(x[1].get_plain_text())
        # print("matmul size:")
        # print(output.size())
        return output
...
class Softmax(Module):
    ...
    def forward(self, input):
        ...
        # print(input.size())
        # print("softmax input:")
        # if TFCrypt.is_encrypted_tensor(input):
        #     print('encrypted')
        #     print(input.get_plain_text())
        # else:
        #     print(input)
            
        result = input.softmax(self.dim)
        # print('result size:')
        # print(result.size())
...
class AdaptiveAvgPool2d(Module):
    ...
    Examples:
        ...
        >>> input = TFCrypt.randn(1, 64, 8, 9)
        >>> output = m(input)
        >>> # target output size of 7x7 (square)
        >>> m = nn.AdaptiveAvgPool2d(7)
        >>> input = TFCrypt.randn(1, 64, 10, 9)
        >>> output = m(input)
...
class BatchNormalization(Module):
    ...
    def _compute_inv_var(self, running_var):
        ...
        if isinstance(running_var, TFCrypt.CrypTensor):
...
class _BatchNorm(Module):
    ...
    def train(self, mode=True):
        ...
        if self.training:
            self.inv_var = None
        elif isinstance(self.running_var, TFCrypt.CrypTensor):
