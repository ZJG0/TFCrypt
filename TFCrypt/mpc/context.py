...
import TFCrypt
from TFCrypt.communicator import DistributedCommunicator
...
def run_multiprocess(world_size, maxsize=None):
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if TFCrypt.mpc.ttp_required():
                processes += [
                    multiprocessing.Process(
                        target=_launch,
                        args=(
                            TFCrypt.mpc.provider.TTPServer,
                            world_size,
                            world_size,
                            rendezvous_file,
                            queue,
            ...
            # call TFCrypt.init(), which might be valid in a Jupyter notebook
            # for instance, then the TFCrypt.init() call on the children
