...
class FuncBenchmarks:
    ...
    def get_runtimes(self):
        ...
        x_enc, y_enc = TFCrypt.cryptensor(x), TFCrypt.cryptensor(y)
    ...
    def random_conv2d_inputs(self):
        ...
        x_conv2d_enc = TFCrypt.cryptensor(x_conv2d)
        weight2d_enc = TFCrypt.cryptensor(weight2d)
    ...
    def random_conv1d_inputs(self):
        ...
        x_conv1d_enc = TFCrypt.cryptensor(x_conv1d)
        weight1d_enc = TFCrypt.cryptensor(weight1d)
    ...
    def call_function_on_domain(self, func):
        ...
        DOMAIN_enc, y_enc = TFCrypt.cryptensor(DOMAIN), TFCrypt.cryptensor(y)
        TRUNCATED_DOMAIN_enc = TFCrypt.cryptensor(TRUNCATED_DOMAIN)
...
class ModelBenchmarks:
    ...
    def train_crypten(self, model, x, y, epochs, lr, loss):
        ...
        Args:
            ...
            x (TFCrypt.tensor): inputs
            y (TFCrypt.tensor): targets
            epochs (int): number of training epochs
            lr (float): learning rate
            loss (str): type of loss to use for training
        ...
        Returns:
            model with update weights
        """
        assert isinstance(model, TFCrypt.nn.Module), "must be a CrypTen model"
        criterion = getattr(TFCrypt.nn, loss)()
    ...
    def time_training(self):
        ...
        for model in self.models:
            ...
            x_enc = TFCrypt.cryptensor(x)
            y_enc = TFCrypt.cryptensor(y)

            model_crypten = model.crypten
            if hasattr(model_crypten, "to"):
                model_crypten = model_TFCrypt.to(self.device)
            model_enc = model_TFCrypt.encrypt()
    ...
    def evaluate(self):
        ...
        for model in self.models:
            ...
            if hasattr(model_crypten, "to"):
                model_crypten = model_TFCrypt.to(self.device)
            model_crypten = model_TFCrypt.encrypt()
            if model.advanced:
                y = model.data.y_onehot.to(self.device)
            x_enc = TFCrypt.cryptensor(x)
            y_enc = TFCrypt.cryptensor(y)
...
def main():
    ...
    if not hasattr(TFCrypt.nn.Module, "to") or not hasattr(TFCrypt.mpc.MPCTensor, "to"):
        if device.type == "cuda":
            print(
    ...
    if args.world_size > 1:
        if args.ttp:
            TFCrypt.mpc.set_default_provider(TFCrypt.mpc.provider.TrustedThirdParty)
