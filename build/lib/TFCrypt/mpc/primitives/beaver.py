...
def __beaver_protocol(op, x, y, *args, **kwargs):
    ...
    # print('Y dim:')
    # print(y.size())
    # with open('matrixDim.txt', 'a+') as writers: # 打开文件
    #     writers.write('X dim:'+'\n')
    #     writers.write (str(x.size())+'\n')
    #     writers.write('Y dim:'+'\n')
    #     writers.write (str(y.size())+'\n')
    #     writers.write('---------------------------------------------------------------'+'\n')
    
    if x.device != y.device:
        raise ValueError(f"x lives on device {x.device} but y on device {y.device}")

    provider = TFCrypt.mpc.get_default_provider()
