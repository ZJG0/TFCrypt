...
import TFCrypt.common  # noqa: F401
import TFCrypt.communicator as comm
import TFCrypt.config  # noqa: F401
import TFCrypt.mpc  # noqa: F401
import TFCrypt.nn  # noqa: F401
import TFCrypt.optim  # noqa: F401
import torch
...
def init(config_file=None, party_name=None, device=None):
    """
    Initialize TFCrypt. It will initialize communicator, setup party
    ...
    comm._init(use_threads=False, init_ttp=TFCrypt.mpc.ttp_required())
    ...
    if comm.get().get_rank() < comm.get().get_world_size():
        _setup_prng()
        if TFCrypt.mpc.ttp_required():
            TFCrypt.mpc.provider.ttp_provider.TTPClient._init()
...
def register_cryptensor(name):
    ...
    from Python cpde, even if the class itself is not  part of TFCrypt. To use
...
def load_from_party(
    ...
    **kwargs,
    ...
    """
    Loads an object saved with `torch.save()` or `TFCrypt.save_from_party()`.
    ...
    Args:
        ...
        `TFCrypt.save_from_party`. Additional kwargs are passed on to the closure.
    """
...
def load(f, load_closure=torch.load, **kwargs):
    """
    Loads shares from an encrypted object saved with `TFCrypt.save()`
    Args:
        f: a file-like object (has to implement `read()`, `readline()`,
              `tell()`, and `seek()`), or a string containing a file name
        ...
        save function in `TFCrypt.save`. Additional kwargs are passed on
        to the closure.
    """
    if "src" in kwargs:
        raise SyntaxError(
            "TFCrypt.load() should not be used with `src` argument. Use load_from_party() instead."
        )
...
def save_from_party(obj, f, src=0, save_closure=torch.save, **kwargs):
    ...
    Args:
        ...
        `TFCrypt.load_from_party`. Additional kwargs are passed on to the closure.
    """
...
def save(obj, f, save_closure=torch.save, **kwargs):
    ...
    Args:
        ...
        save_closure: Custom save function that matches the interface of `torch.save`,
        to be used when the tensor is saved with a custom load function in
        `TFCrypt.load`. Additional kwargs are passed on to the closure.
    """
...
def stack(tensors, dim=0):
    ...
    `TFCrypt.cat`, this adds a dimension to the result tensor.
    """
...
def trace(tracing=True):
    TFCrypt.mpc.get_default_provider().trace(tracing=tracing)


def trace_once():
    TFCrypt.mpc.get_default_provider().trace_once()


def fill_cache():
    TFCrypt.mpc.get_default_provider().fill_cache()
