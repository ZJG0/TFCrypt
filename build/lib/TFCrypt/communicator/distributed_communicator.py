...
from TFCrypt.common import serial
...
class DistributedCommunicator(Communicator):
    ...
    def initialize(cls, rank, world_size, init_ttp=False):
        ...
        if os.name == "nt":
            raise OSError(
                ...
                + "Please initialize CrypTen via TFCrypt.init_thread() instead."
            )
    ...
    def send(self, tensor, dst, tag=0):
        ...
        dist.send(tensor.data, dst, group=self.main_group, tag=tag)
    ...
    def recv(self, tensor, src=None, tag=0):
        ...
        assert dist.is_initialized(), "initialize the communicator first"
        result = tensor.clone()
        dist.recv(result.data, src=src, group=self.main_group, tag=tag)
        return result
    ...
    def isend(self, tensor, dst, tag=0):
        """Sends the specified tensor to the destination dst."""
        assert dist.is_initialized(), "initialize the communicator first"
        return dist.isend(tensor.data, dst, group=self.main_group, tag=tag)

    @_logging
    def irecv(self, tensor, src=None, tag=0):
        """Receives a tensor from an (optional) source src."""
        assert dist.is_initialized(), "initialize the communicator first"
        return dist.irecv(tensor.data, src=src, group=self.main_group, tag=tag)
