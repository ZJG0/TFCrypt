...
def randn(*sizes, device=None):
    ...
    """
    u = TFCrypt.rand(*sizes, device=device).flatten()
    odd_numel = u.numel() % 2 == 1
    if odd_numel:
        u = TFCrypt.cat([u, TFCrypt.rand((1,), device=device)])
...
def bernoulli(self):
    ...
    input tensor."""
    return self > TFCrypt.rand(self.size(), device=self.device)
...
def weighted_index(self, dim=None):
    ...
    r = TFCrypt.rand(max_weight.size(), device=self.device) * max_weight
