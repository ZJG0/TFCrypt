...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, Q, K, V, attn_mask):
        ...
        # print("------------------------before-------------------")
        # print(scores)
        masked_scores = scores.masked_fill_(attn_mask, -1e9) # Fills elements of self tensor with value where mask is True.
        # print("------------------------after-------------------")
        # print(scores)
        # scores[attn_mask>0] = -1e9
        # attn_size = scores.size()
        # mask_ = attn_mask.reshape(-1)
        # attn_ = scores.reshape(-1)
        # for i, value in enumerate(mask_):
        #     if value:
        #         attn_[i] = -1e9
        
        # scores = attn_.reshape(attn_size)
        
        attn = nn.Softmax(dim=-1)(masked_scores)
